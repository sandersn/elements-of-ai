...
(def diff-const-rule
  (->rule '(d (? e1) (diff-const-e2-rule e2))
   (fn [d] 0)
   "diff-const-rule"))
...
(def diff-product-rule
  (->rule '(d (diff-product-e3-rule e3) (? v1))
   (fn [d] (list '+
...
(defn diff-power-e3-rule [d x]
  (and (not (atom? x))
       (when-let [d2 (match-state '(exp (? e1) (number-state? e2)) x)]
         (merge d d2))))
(def diff-power-rule
  (->rule '(d (diff-power-e3-rule e3) (? v1))
   (fn [d] (list '* (d 'e2)
...
(def lprod-zero-rule (->rule '(* (? x) 0) (fn [d] 0) "lprod-zero-rule"))
(def rprod-zero-rule (->rule '(* 0 (? x)) (fn [d] 0) "rprod-zero-rule"))
(def lprod-id-rule (->rule '(* (? x) 1) #(% 'x) "lprod-id-rule"))
(def rprod-id-rule (->rule '(* 1 (? x)) #(% 'x) "rprod-id-rule"))
(def prod-const-rule (->rule '(* (number-state? x) (number-state? y)) #(* (% 'x) (% 'y)) "prod-const-rule"))
(def exp-zero-rule (->rule '(exp (? x) 0) (fn [d] 1) "exp-zero-rule"))
(def exp-id-rule (->rule '(exp (? x) 1) #(% 'x) "exp-id-rule"))
(def dec-rule (->rule '(dec (number-state? x)) #(dec (% 'x)) "dec-rule"))
...
(defn fire [rule d]
  ;(println (:name rule) "fires.")
  ((:action rule) d))
(defn try-rule [rule expression]
  (if (atom? expression)
    nil
    (if-let [d (match-state (:pattern rule) expression)]
      (fire rule d)
